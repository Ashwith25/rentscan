/**
 * extractor-cs.js — Content Script Edition
 *
 * Same logic as extractor.js but wrapped in an IIFE and exposed as
 * window.RentScan.extractListing — no ES module export syntax.
 *
 * Loaded via manifest content_scripts before content.js.
 * background.js continues to use extractor.js (the ES module version).
 */
(() => {
  ...
  function extractAvailability(text) {
    ...
    for (const p of patterns) {
      p.lastIndex = 0;
      const m = p.exec(text);
      if (m) return m[1] ? m[1].trim() : 'Immediately';
    }
    return null;
  }
  ...
  function extractLocation(text) {
    const patterns = [
      ...
      /([A-Z][a-zA-Z]+(?:\s+[A-Z][a-zA-Z]+){0,2})\s+(?:area|neighborhood|district|zip)/gi,
      /address:?\s*(.+?)(?:\.|,|\n|$)/gi,
    ];
    ...
    for (const p of patterns) {
      ...
      if (m && m[1]) {
        const loc = m[1].trim();
        if (!stopWords.has(loc.split(' ')[0]) && loc.length > 2 && loc.length < 60) return loc;
      }
    }
    return null;
  }
  ...
  function extractFurnishedStatus(text) {
    ...
    for (const [status, kws] of Object.entries(FURNISHED_KEYWORDS)) {
      if (kws.some(k => lt.includes(k))) return status;
    }
    return null;
  }
  ...
  function extractLeaseType(text) {
    const lt = text.toLowerCase();
    for (const [type, kws] of Object.entries(LEASE_TYPE_KEYWORDS)) {
      if (kws.some(k => lt.includes(k))) return type;
    }
    return null;
  }
  ...
  function extractAmenities(text) {
    const lt = text.toLowerCase();
    const found = [];
    for (const [amenity, kws] of Object.entries(AMENITIES_MAP)) {
      if (amenity !== 'furnished' && kws.some(k => lt.includes(k))) {
        found.push(amenity);
      }
    }
    return found;
  ...
  function extractPropertyType(text) {
    const lt = text.toLowerCase();
    if (/studio|efficiency apartment/.test(lt)) return 'Studio';
    if (/\d+\s*bhk/.test(lt)) {
      const m = lt.match(/(\d+)\s*bhk/);
      return m ? `${m[1]}BHK` : 'Apartment';
    }
    if (/condo/.test(lt)) return 'Condo';
    if (/townhouse|town house/.test(lt)) return 'Townhouse';
    if (/house|home/.test(lt)) return 'House';
    if (/flat/.test(lt)) return 'Flat';
    if (/room(?:\s+in)?/.test(lt)) return 'Room';
    if (/apartment|apt/.test(lt)) return 'Apartment';
    return 'Property';
  }
  ...
  function computeConfidence(text, extracted) {
    ...
    const matchesSell = SELL_TRIGGER_KEYWORDS.some(kw => lt.includes(kw));
    const matchesStrongRental = STRONG_RENTAL_KEYWORDS.some(kw => lt.includes(kw));
    ...
    if (lt.includes('for rent') || lt.includes('to let')) score += 25;
    if (lt.includes('available')) score += 10;
    if (extracted.availability) score += 10;
    if (extracted.amenities.length > 0) score += 10;
    const triggerCount = RENTAL_TRIGGER_KEYWORDS.filter(kw => lt.includes(kw)).length;
  ...
  function generateId(msg) {
    ...
    const ts = (msg.date instanceof Date ? msg.date : new Date()).getTime();
    return `listing_${Math.abs(hash)}_${ts}`;
  }
  ...
  // Expose on global namespace — content.js reads this
  window.RentScan = window.RentScan || {};
  window.RentScan.extractListing = extractListing;
})();
